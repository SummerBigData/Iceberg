...
def getcnn(imgsize):
	...
	def init():
    		return initializers.RandomUniform(minval=-0.1, maxval=0.1, seed=None)

	#Building the model
	gmodel=Sequential()
	#Conv Layer 1
	gmodel.add(Conv2D(64,kernel_size=(3, 3),activation='relu',input_shape=(imgsize,imgsize,3),kernel_initializer=init() ))
	gmodel.add(MaxPooling2D(pool_size=(3, 3), strides=(2, 2)))
	gmodel.add(Dropout(0.2))
	
	#Conv Layer 2
	gmodel.add(Conv2D(128, kernel_size=(3, 3), activation='relu',kernel_initializer=init() ))
	...
	#Conv Layer 4
	gmodel.add(Conv2D(64, kernel_size=(3, 3), activation='relu',kernel_initializer=init() ))
	gmodel.add(MaxPooling2D(pool_size=(2, 2), strides=(2, 2)))
	gmodel.add(Dropout(0.2))
	...
	#Dense Layers
	gmodel.add(Dense(512,kernel_initializer=init() ))
	gmodel.add(Activation('relu'))
	#gmodel.add(Dropout(0.2))

	#Dense Layer 2
	gmodel.add(Dense(256,kernel_initializer=init() ))
	gmodel.add(Activation('relu'))
	#gmodel.add(Dropout(0.2))
