...
def shuffleData3(xtr, ytr, ang):
	imgsize = xtr.shape[1]
	# Linearize the x features and columnize the y data
	xtr = xtr.reshape((xtr.shape[0], imgsize*imgsize*3))
	ytr = ytr.reshape((xtr.shape[0], 1))
	ang = ang.reshape((xtr.shape[0], 1))
	# Stick them together and shuffle them
	augdat = np.hstack((ytr, ang, xtr))	#(2000, 16876)
	np.random.shuffle(augdat)
	# Seperate augdat and reshape back to original dimensions
	xtrshuffle = augdat[:, 2:].reshape((xtr.shape[0], imgsize, imgsize, 3))
	ytrshuffle = np.ravel(augdat[:, 0])
	angshuffle = np.ravel(augdat[:, 1])
	return xtrshuffle, ytrshuffle, angshuffle
...
def dataprepAngle():
	...
	# Read out the data in the two bands, as 1604 x 75 x 75 arrays
	TRb1, TRb2, TRname, TRlabel, TRangle, TRonlyAngle = DataSort(train)
	ang = np.zeros((TRangle.shape[0]))
	for i in range(TRb1.shape[0]):
		if TRangle[i] == 'na':
			ang[i] = 0
		else:
			ang[i] = TRangle[i]
	# DATA PREP
	xb1 = TRb1.reshape((TRb1.shape[0], 75, 75, 1))
	xb2 = TRb2.reshape((TRb2.shape[0], 75, 75, 1))
	xbavg = (xb1 + xb2) / 2.0
	#xbavg = np.zeros(xb1.shape)
	x = np.concatenate((xb1, xb2, xbavg ), axis=3)
	
	# Shuffle data
	x, TRlabel, ang = shuffleData3(x, TRlabel, ang)

	xtr = x[:1203,:,:,:]
	xte = x[1203:,:,:,:]
	atr = ang[:1203].astype(float)
	ate = ang[1203:].astype(float)
